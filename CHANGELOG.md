...
- E2E diag redaction: `emitDiag` scrubs payloads before serialisation — sensitive key names (`password`, `*token*`, `*secret*`, `api_key`, `authorization`, `cookie`, `service_role`, `anon_key`, etc.) become `[REDACTED]`, and value-level regexes strip JWTs, `Bearer …` headers, `sk-…` API keys, Telegram bot tokens, GitHub PATs, and long hex/base64 blobs from `message` / `details` / `hint` / nested rows. Covered by `e2e/diag.test.ts`. See [`docs/e2e-diag-redaction-contract.md`](docs/e2e-diag-redaction-contract.md) for the full contract (what is scrubbed, what is allowed, examples, and how to change it safely).
- E2E CI defence-in-depth: new `scripts/scrub-e2e-logs.ts` re-runs the same redactor over both `e2e-logs/raw.log` and `e2e-logs/diagnostics.jsonl` before the GitHub step summary is rendered and before the `e2e-logs/` artefact is uploaded, so the summary table, the collapsible JSON block, and the artefact are all guaranteed-clean even if a non-`emitDiag` code path printed a secret-looking string.
- E2E redaction contract test (`e2e/diag-contract.test.ts`): parses the three worked examples in `docs/e2e-diag-redaction-contract.md` and asserts (1) `redact()` of Example 1's "before" deep-equals its "after", (2) `scrubString()` of Example 2's Bearer/JWT line matches verbatim, (3) `scripts/scrub-e2e-logs.ts` run against a temp dir rewrites `raw.log` to Example 3's "after" and re-redacts a JSONL record with both a sensitive key and a Bearer value. Also pins the key-pattern and value-pattern table rows so doc and code can't drift apart silently.
