...
- [AWIP-as-MoE (ADR-0007)](mem://features/awip-as-moe) — manifest=router (Control Plane/modules only), tenant_node ancestry=skim, per-capability verifiers=additive promotion gate; intent only, build trigger = ≥1 module with real traffic; Part 2 (expert-feedback) deferred
...
- [Secret rotation safety](mem://features/secret-rotation-safety) — AWIP_SERVICE_TOKEN mirrored across env + app_secrets + vault.secrets; secrets-health-check sync modes (env-to-db / env-to-vault / env-to-all) + operator JWT gated; `/admin/secrets-health` UI + runbook `docs/runbooks/awip-service-token-rotation.md`
...
- [HTML as renderable view](mem://preferences/html-as-view) — HTML reports for Deep Audit + AWIP Reviews only; markdown stays source of truth for docs/mem/plans/ADRs/CHANGELOG
