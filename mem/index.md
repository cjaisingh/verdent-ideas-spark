...
Git provider: project IS mirrored to `cjaisingh/verdent-ideas-spark`. Failures on `main` are caught autonomously within 5min by `gh-actions-watch` → sentinel + Telegram; `ci-status-sync` only watches workflows tied to open discussion_actions. Edits to `supabase/functions/*` and `eslint.config.js` land in the mirror.
