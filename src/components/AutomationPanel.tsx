...
import { ShieldCheck, FlaskConical, ClipboardCheck } from "lucide-react";
...
export const AutomationPanel = () => {
  ...
  const runReview = async () => {
    const { error } = await supabase.functions.invoke("scheduled-code-review", { body: {} });
    if (error) console.error(error);
    setTimeout(load, 1500);
  };
  const runQa = async () => {
    const { error } = await supabase.functions.invoke("qa-validate", { body: {} });
    if (error) console.error(error);
    setTimeout(load, 1500);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      ...
      <section className="rounded-md border border-border bg-card p-3 space-y-2">
        <header className="flex items-center justify-between">
          ...
          <button onClick={runReview} className="text-[10px] text-muted-foreground hover:text-foreground underline-offset-2 hover:underline">Run now</button>
        </header>
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">openai/gpt-5 · weekly Mon 06:00 UTC</div>
        {findings.length === 0 ? (
      ...
      {/* Test runs */}
      <section className="rounded-md border border-border bg-card p-3 space-y-2">
        <header className="flex items-center gap-2 text-sm font-medium"><FlaskConical className="h-4 w-4" /> Tests</header>
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">nightly 02:00 UTC via GitHub Actions</div>
        {runs.length === 0 ? (
      ...
      <section className="rounded-md border border-border bg-card p-3 space-y-2">
        <header className="flex items-center justify-between">
          ...
          <button onClick={runQa} className="text-[10px] text-muted-foreground hover:text-foreground underline-offset-2 hover:underline">Run probes</button>
        </header>
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">weekly Fri 16:00 UTC</div>
