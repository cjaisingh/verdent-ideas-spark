...
DECLARE
  ...
  order_clause text;
BEGIN
  -- Authorisation enforced by the db-explorer edge function (operator role check)
  -- and by EXECUTE being revoked from anon/authenticated. When called via the
  -- service role from the edge function, auth.uid() is NULL — so we only run
  -- the role check for non-NULL callers (authenticated direct callers would
  -- still be blocked at the GRANT layer, this is belt-and-braces).
  IF auth.uid() IS NOT NULL
     AND NOT (public.has_role(auth.uid(), 'operator'::app_role)
