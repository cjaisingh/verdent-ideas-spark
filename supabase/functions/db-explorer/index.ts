...
// emitting an audit line. Matched case-insensitively as a substring so e.g.
// `access_token`, `apiKey`, `Authorization`, `cookie`, `secret_value` all hit.
...
// Patterns inside string values that look like credentials; full string is replaced.
const SENSITIVE_VALUE_RES: RegExp[] = [
  /\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\b/, // JWT
  /\b(sk|pk|rk|sbp)_[A-Za-z0-9]{16,}\b/,                                // stripe/supabase-style keys
...
// Error/RPC messages can leak SQL fragments, identifiers from other schemas,
// or table data. Keep only a short, generic shape.
...
function audit(e: AuditEntry) {
  const safe: AuditEntry = {
    ...e,
    rejection_reason: redactErrorMessage(e.rejection_reason),
    error_code: redactErrorMessage(e.error_code),
    requested: e.requested ? (redactValue(e.requested) as Record<string, unknown>) : null,
  };
  // Single-line JSON keeps it easy to filter in the logs UI.
  console.log("audit " + JSON.stringify({ kind: "db_explorer_audit", ...safe }));
}
