...
const SYSTEM_PROMPT = `You extract concrete, actionable follow-ups from a code-review discussion transcript.
Return STRICT JSON only — no prose, no markdown — matching:
{"actions":[{"title":"<<=80 chars>","details":"<optional 1-3 sentences>","priority":"low|med|high","owner_hint":"<optional>","confidence":0.0}]}
Rules:
- Each action must be a discrete next step a human can do (e.g. "Add RLS policy on X", not "consider security").
- Skip vague intent or already-done items.
...
Deno.serve(withLogger("discussion-extract-actions", async (req) => {
  ...
  try {
    ...
    const { data: disc } = await admin
      ...
      .select("id, subject_type, subject_id, title")
      .eq("id", discussionId)
      .maybeSingle();
    if (!disc) return json({ error: "discussion not found" }, 404);
    ...
    if (!upstream.ok) {
      ...
      await logAiCall(admin, { job: "discussion-extract-actions", model, trigger: "user", startedAt: aiStart, response: upstream, errorText: t, request_ref: { discussion_id: discussionId } });
    ...
    await logAiCall(admin, { job: "discussion-extract-actions", model, trigger: "user", startedAt: aiStart, response: upstream, json: out, request_ref: { discussion_id: discussionId } });
    ...
    return json({ proposals, subject_type: disc.subject_type, subject_id: disc.subject_id });
  } catch (e) {
