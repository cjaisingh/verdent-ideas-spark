...
// posts to the operator webhook. Mirrors the in-function copies in
// qa-validate / scheduled-code-review / record-test-run / secrets-health-check
// so every job can surface auth/operational failures the same way.
//
...
export async function dispatchAlert(
  ...
  try {
    ...
    const webhookConfigured = !!settings?.enabled && !!settings?.webhook_url;

    // Dedupe before delivering so a hot loop doesn't spam the webhook.
    if (webhookConfigured && !flagOff) {
  ...
  // Always record the attempt — even if the webhook is off, the operator
  // can see in alert_log that the condition fired.
  try {
